...
class File:
    ...
    async def ShowContent_File(self, FileName: Path) -> str:
        ...
        try:
            if FileName.is_file():
                async with aiofiles.open(FileName, mode='r', encoding='utf-8') as content:
                ...
                if search_type in ('folders', 'both'):
                    for folder in dirs:
                        if folder.startswith(query):
                            ...
                            matches_table.add_row(str(os.path.join(root,folder)),'Folder')

                if search_type in ('files', 'both'):
                    for file in files:
                        if file.startswith(query):
                            matches.append(os.path.join(root, file))
                            matches_table.add_row(str(os.path.join(root,folder)),'File')
    ...
    async def Create_Folder(self, Source: Path) -> str:
        ...
        try:
            ...
            if Source.exists():
                return f"[error]{Source.name} Already Exists in {Source.resolve().parent}"
