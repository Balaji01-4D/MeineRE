...
class System:
    ...
    async def DiskSpace(self, Destination: Path = Path('/')) -> Panel:
        try:
            ...
            disk_usage, swap_memory = await asyncio.gather(disk_usage_task, swap_memory_task)
            ...
            available_task = progress.add_task("[#2196F3]AVAILABLE %", total=100, completed=available_percentage)
            used_task = progress.add_task("[#2196F3]USED %", total=100, completed=used_percentage)
            ...
            storage_table.add_row('[#2196F3]TOTAL', SizeHelper(total), SizeHelper(swap_memory.total))
            storage_table.add_row('[#2196F3]USED', SizeHelper(used), SizeHelper(swap_memory.used))
            storage_table.add_row('[#2196F3]FREE', SizeHelper(free), SizeHelper(swap_memory.free))
    ...
    async def Info(self, Name: Path) -> Table | str:
        ...
        try:
            stats, fullpath = await asyncio.gather(
                asyncio.to_thread(Name.stat),
                asyncio.to_thread(Name.resolve)
            )
    ...
    async def HomeDir(self) -> Panel:
        return Panel(f'Home Directory :  {Path.home()}',expand=False,style='color(105)')
    ...
    async def RAMInfo(self) -> Panel:
        ...
        rampanel.add_task("[#C5C8C6]AVAILABLE % ", total=100, completed=data['AVAILABLE'])
...
#final
    async def SYSTEM(self) -> Panel:
        ...
        systemtable = Table(show_header=False, show_lines=True, title="[#61AFEF]SYSTEM INFO", border_style='#ABB2BF')
    ...
    async def Battery(self) -> Panel:
        ...
        if battery:
            ...
            status_message = f"[#61AFEF]{'Charging' if battery.power_plugged else 'Not Charging'}"
            status = Panel(f"[#C5C8C6]Battery Status: {status_message}", expand=False, border_style='#ABB2BF')
            gp = Group(progress, status)
            return Panel(gp, expand=False, border_style='#ABB2BF')
        
        return Panel(f"[bold green]No battery information available.", expand=False, border_style='#ABB2BF')
    ...
    async def CPU(self) -> Panel:
        ...
        Task = progress.add_task(f"[#C5C8C6]  CPU PERCENT % ", total=100, completed=Usage)
    ...
    async def USER(self) -> Panel:
        import getpass
        return Panel(f"[#C5C8C6]Current User:[#61AFEF] {getpass.getuser()}",expand=False,border_style='#ABB2BF')

    async def DiskInfo(self):
        tableofdisk = Table(show_lines=True,border_style='#ABB2BF',title='[#61AFEF]Disk Info')
    ...
    async def Processes():
        ...
        for proc in psutil.process_iter(attrs=['pid', 'name', 'status', 'memory_info']):
            try:
                ...
                tableofproccess.add_row(str(pid), name, str(status), f"{memory:.2f} MB", f"{cpu_usage:.2f}%",style='#98C379')
