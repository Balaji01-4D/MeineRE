...
def _calculate_key1(password: bytes, cycles: int, salt: bytes, digest: str) -> bytes:
    ...
    if cycles == 0x3F:
        ...
        for round in range(rounds):
            m.update(
                salt + password + round.to_bytes(8, byteorder="little", signed=False)
            )
        key = m.digest()[:32]
    return key
                ...
                m.update(
                    memoryview(
                        b"".join(
                            [
                                saltpassword
                                + (s + i).to_bytes(8, byteorder="little", signed=False)
...
def readlink(
    path: Union[str, pathlib.Path], *, dir_fd=None
...
def is_path_valid(target: pathlib.Path, parent: pathlib.Path) -> bool:
    ...
    if parent.is_absolute():
        ...
        return is_relative_to(
            canonical_path(target), pathlib.Path.cwd().joinpath(parent)
        )
...
def check_win32_file_namespace(pathname: pathlib.Path) -> pathlib.Path:
    ...
    if (
        ...
        and pathname.is_absolute()
        and not is_windows_unc_path(pathname)
    ):
