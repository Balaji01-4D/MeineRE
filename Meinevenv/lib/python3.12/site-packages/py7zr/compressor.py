...
from py7zr.properties import (COMPRESSION_METHOD, FILTER_ARM, FILTER_ARMTHUMB,
                              FILTER_BROTLI, FILTER_BZIP2, FILTER_COPY,
                              FILTER_CRYPTO_AES256_SHA256, FILTER_DEFLATE,
                              FILTER_DEFLATE64, FILTER_DELTA, FILTER_IA64,
                              FILTER_LZMA, FILTER_LZMA2, FILTER_POWERPC,
                              FILTER_PPMD, FILTER_SPARC, FILTER_X86,
                              FILTER_ZSTD, MAGIC_7Z, CompressionMethod,
...
class ISevenZipDecompressor(ABC):
    @abstractmethod
    def decompress(
        self, data: Union[bytes, bytearray, memoryview], max_length: int = -1
    ) -> bytes:
        """
...
class AESCompressor(ISevenZipCompressor):
    ...
    def __init__(self, password: str, blocksize: Optional[int] = None) -> None:
        ...
        key = calculate_key(
            password.encode("utf-16LE"), self.cycles, self.salt, "sha256"
        )
        self.iv += bytes(
            self.AES_CBC_BLOCKSIZE - len(self.iv)
        )  # zero padding if iv < AES_CBC_BLOCKSIZE
    ...
    def encode_filter_properties(self):
        ...
        secondbyte = (
            ((ivsize - 1) & 0x0F) + (((saltsize - saltfirst) << 4) & 0xF0)
        ).to_bytes(1, "little")
...
class AESDecompressor(ISevenZipDecompressor):
    ...
    def __init__(
        self, aes_properties: bytes, password: str, blocksize: Optional[int] = None
    ) -> None:
        ...
        if firstbyte & 0xC0 != 0:
            ...
            key = calculate_key(
                password.encode("utf-16LE"), numcyclespower, salt, "sha256"
            )
    ...
    def compress(
        ...
        if not self._enabled:
            raise UnsupportedCompressionMethodError(
                None, "deflate64 is disabled on pypy."
            )
...
class PpmdDecompressor(ISevenZipDecompressor):
    ...
    def decompress(
        self, data: Union[bytes, bytearray, memoryview], max_length=-1
    ) -> bytes:
    ...
    def encode_filter_properties(cls, filter: Dict[str, Union[str, int]]):
        ...
        if isinstance(mem, str):
            if mem.isdecimal():
                ...
                raise ValueError(
                    "Ppmd:Unsupported memory size is specified: {0}".format(mem)
                )
...
class SevenZipDecompressor:
    ...
    def __init__(
        ...
        if len(coders) > 4:
            raise UnsupportedCompressionMethodError(
                coders,
                "Maximum cascade of filters is 4 but got {}.".format(len(coders)),
            )
        self.methods_map = [
        ...
        if SupportedMethods.needs_password(coders) and password is None:
            raise PasswordRequired(
                coders, "Password is required for extracting given archive."
            )
        ...
        if len(coders) >= 2:
            ...
            for i, coder in enumerate(coders):
                ...
                if (
                    SupportedMethods.is_compressor_id(filter_id)
                    and filter_id != FILTER_LZMA2
                ):
        ...
        if all(self.methods_map):
            ...
            for i in range(len(coders)):
                if (not any(self.methods_map[:i])) and all(self.methods_map[i:]):
                    ...
                    self.chain.append(
                        self._get_lzma_decompressor(coders[i:], unpacksizes[i])
                    )
                    break
            else:
                for i in range(len(coders)):
            ...
            raise UnsupportedCompressionMethodError(
                coders, "Combination order of methods is not supported."
            )
    ...
    def _get_lzma_decompressor(self, coders: List[Dict[str, Any]], unpacksize: int):
        ...
        for coder in coders:
            if coder["numinstreams"] != 1 or coder["numoutstreams"] != 1:
                raise UnsupportedCompressionMethodError(
                    coders, "Only a simple compression method is currently supported."
                )
    ...
    def _get_alternative_decompressor(
        ...
        bz2.BZ2Decompressor, lzma.LZMADecompressor, ISevenZipDecompressor
    ]:  # noqa
        filter_id = SupportedMethods.get_filter_id(coder)
        ...
        if SupportedMethods.is_native_coder(coder):
            raise UnsupportedCompressionMethodError(
                coder, "Unknown method code:{}".format(coder["method"])
            )
        if filter_id not in algorithm_class_map:
            raise UnsupportedCompressionMethodError(
                coder, "Unknown method filter_id:{}".format(filter_id)
            )
        if algorithm_class_map[filter_id][1] is None:
            raise UnsupportedCompressionMethodError(
                coder,
                "Decompression is not supported by {}.".format(
                    SupportedMethods.get_method_name_id(filter_id)
                ),
            )
        #
        if SupportedMethods.is_crypto_id(filter_id):
            return algorithm_class_map[filter_id][1](
                coder["properties"], password, self.block_size
            )
        elif SupportedMethods.need_property(filter_id):
            return algorithm_class_map[filter_id][1](
                coder["properties"], self.block_size
            )
        else:
            return algorithm_class_map[filter_id][1]()
...
class SevenZipCompressor:
    ...
    def __init__(self, filters=None, password=None, blocksize: Optional[int] = None):
        ...
        if len(self.filters) > 4:
            raise UnsupportedCompressionMethodError(
                filters,
                "Maximum cascade of filters is 4 but got {}.".format(len(self.filters)),
            )
        self.methods_map = [
            SupportedMethods.is_native_filter(filter) for filter in self.filters
        ]
        ...
        if filter_id not in algorithm_class_map:
            ...
            if filter_id == FILTER_ZSTD:
                ...
                properties = struct.pack(
                    "BBBBB",
                    pyzstd.zstd_version_info[0],
                    pyzstd.zstd_version_info[1],
                    level,
                    0,
                    0,
                )
...
class SupportedMethods:
    ...
    def raise_unsupported_filter_id(cls, filter_id):
        raise UnsupportedCompressionMethodError(
            filter_id,
            "Found an unsupported filter id is specified."
