...
def write_boolean(
    file: Union[BinaryIO, WriteWithCrc], booleans: List[bool], all_defined: bool = False
):
...
class PackInfo:
    ...
    def _read(self, file: BinaryIO):
        ...
        if pid != PROPERTY.END:
            raise Bad7zFile(
                "end id expected but %s found" % repr(pid)
            )  # pragma: no-cover  # noqa
        self.packpositions = [
            sum(self.packsizes[:i]) for i in range(self.numstreams + 1)
        ]  # type: List[int]
    ...
    def write(self, file: Union[BinaryIO, WriteWithCrc]):
        ...
        self.enable_digests = functools.reduce(
            operator.or_, self.digestdefined, self.enable_digests
        )
...
class UnpackInfo:
    ...
    def _read(self, file: BinaryIO):
        ...
        if pid != PROPERTY.FOLDER:
            raise Bad7zFile(
                "folder id expected but %s found" % repr(pid)
            )  # pragma: no-cover
    ...
    def _retrieve_coders_info(self, file: BinaryIO):
        ...
        if pid != PROPERTY.CODERS_UNPACK_SIZE:
            raise Bad7zFile(
                "coders unpack size id expected but %s found" % repr(pid)
            )  # pragma: no-cover
        ...
        if pid != PROPERTY.END:
            raise Bad7zFile(
                "end id expected but 0x{:02x} found at 0x{:08x}".format(
                    ord(pid), file.tell()
                )
            )  # pragma: no-cover  # noqa
...
class SubstreamsInfo:
    ...
    def _read(self, file: BinaryIO, numfolders: int, folders: List[Folder]):
        ...
        if pid == PROPERTY.CRC:
            ...
            for i in range(numfolders):
                ...
                if (
                    numsubstreams == 1
                    and folder.digestdefined
                    and folder.crc is not None
                ):
    ...
    def write(self, file: Union[BinaryIO, WriteWithCrc]):
        if (
            len(self.num_unpackstreams_folders) == 0
        ):  # pragma: no-cover  # nothing to write
            return
        write_byte(file, PROPERTY.SUBSTREAMS_INFO)
        solid = functools.reduce(
...
class FilesInfo:
    ...
    def _write_times(
        self, fp: Union[BinaryIO, WriteWithCrc], propid, name: str
    ) -> None:
    ...
    def _write_prop_bool_vector(
        self, fp: Union[BinaryIO, WriteWithCrc], propid, vector
    ) -> None:
        write_byte(fp, propid)
    ...
    def _write_names(self, file: Union[BinaryIO, WriteWithCrc]):
        ...
        for f in self.files:
            if f.get("filename", None) is not None:
                ...
                name_size += (
                    len(f["filename"].encode("utf-16LE")) + 2
                )  # len(str + NULL_WORD)
        if name_defined > 0:
    ...
    def initialize(self):
        if not self._initialized:
            ...
            if self.main_streams is not None:
                ...
                self.main_streams.substreamsinfo.num_unpackstreams_folders = [
                    len(folders)
                ]
            ...
            if (
                self.main_streams is not None
                and self.main_streams.unpackinfo is not None
            ):
