...
class Cli:
    ...
    def _create_parser(self):
        ...
        subparsers = parser.add_subparsers(
            title="subcommands", help=COMMAND_HELP_STRING
        )
        ...
        extract_parser.add_argument(
            "arcfile", type=pathlib.Path, help="7z archive file"
        )
    ...
    def run_list(self, args):
        ...
        if re.fullmatch(r"[.]0+1?", target.suffix):
            ...
            with multivolumefile.MultiVolume(
                mv_target,
                mode="rb",
                ext_digits=len(target.suffix) - 1,
                ext_start=ext_start,
            ) as mvf:
    ...
    def _run_list(self, target, verbose):
        ...
        with py7zr.SevenZipFile(target, "r") as a:
            ...
            archive_list = a.list()
            if verbose:
                if isinstance(target, io.FileIO) or isinstance(
                    target, multivolumefile.MultiVolume
                ):
    ...
    def run_extract(self, args: argparse.Namespace) -> int:
        ...
        if verbose:
            archive_info = a.archiveinfo()
            cb = CliExtractCallback(
                total_bytes=archive_info.uncompressed, ofd=sys.stderr
            )
        try:
            if args.odir:
    ...
    def run_create(self, args):
        ...
        volume_size = (
            args.volume[0] if getattr(args, "volume", None) is not None else None
        )
        ...
        if volume_size is None:
            ...
            with multivolumefile.MultiVolume(
                target, mode="wb", volume=size, ext_digits=4
            ) as mvf:
