...
from typing import (IO, Any, BinaryIO, Collection, Dict, List, Optional, Tuple,
                    Type, Union)
...
from py7zr.exceptions import (AbsolutePathError, Bad7zFile, CrcError,
                              DecompressionError, InternalError,
                              UnsupportedCompressionMethodError)
from py7zr.helpers import (ArchiveTimestamp, MemIO, NullIO, calculate_crc32,
                           check_archive_path, filetime_to_dt,
                           get_sanitized_output_path, is_path_valid, readlink)
from py7zr.properties import (DEFAULT_FILTERS, FILTER_DEFLATE64, MAGIC_7Z,
                              get_default_blocksize, get_memory_limit)
...
class SevenZipFile(contextlib.AbstractContextManager):
        ...
        *,
        ...
        try:
            if mode == "r":
                ...
                self.fp.seek(
                    self.afterheader
                )  # seek into start of payload and prepare worker to extract
    ...
    def _real_get_contents(self, password) -> None:
        ...
        if (
            hasattr(self.header, "main_streams")
            and self.header.main_streams is not None
        ):
        ...
        for file_id, file_info in enumerate(self.header.files_info.files):
            if not file_info["emptystream"] and folders is not None:
                ...
                numinstreams = max(
                    [coder.get("numinstreams", 1) for coder in folder.coders]
                )
                ...
                if folder is None:
                    ...
                    if pstat.input >= subinfo.num_unpackstreams_folders[pstat.folder]:
                        ...
                        pstat.src_pos += sum(
                            packinfo.packsizes[
                                pstat.stream : pstat.stream + numinstreams
                            ]
                        )
                        pstat.folder += 1
        ...
        if not self.password_protected and self.header.main_streams is not None:
            ...
            self.password_protected = any(
                [
                    SupportedMethods.needs_password(folder.coders)
    ...
    def _extract(
        ...
        for f in self.files:
            if targets is not None and recursive is False:
                ...
                if f.filename not in targets and not any(
                    [f.filename.startswith(target) for target in targets]
                ):
                    self.worker.register_filelike(f.id, None)
                    continue
            ...
            if path is None or path.is_absolute():
                ...
                outfilename = get_sanitized_output_path(
                    outname, pathlib.Path(os.getcwd()).joinpath(path)
                )
            if return_dict:
            ...
            try:
                ...
                if target_dir.is_dir():
                    ...
                    raise DecompressionError(
                        "Directory {} is existed as a normal file.".format(
                            str(target_dir)
                        )
                    )
                else:
                    raise DecompressionError(
                        "Directory {} making fails on unknown condition.".format(
                            str(target_dir)
                        )
                    )
            ...
            for f in filters:
                if f["id"] == FILTER_DEFLATE64:
                    raise UnsupportedCompressionMethodError(
                        filters, "Compression with deflate64 is not supported."
                    )
    ...
    def _get_method_names(self) -> List[str]:
        try:
            ...
            raise UnsupportedCompressionMethodError(
                self.header.main_streams.unpackinfo.folders, "Unknown method"
            )
    ...
    @staticmethod
    def _make_file_info(
        target: pathlib.Path, arcname: Optional[str] = None, dereference=False
    ) -> Dict[str, Any]:
        f: Dict[str, Any] = {}
        f["origin"] = target
        ...
        if sys.platform == "win32":
            ...
            if target.is_symlink():
                if dereference:
                    ...
                    f["emptystream"] = False
                    f["attributes"] = getattr(stat, "FILE_ATTRIBUTE_ARCHIVE") | getattr(
    ...
    def archiveinfo(self) -> ArchiveInfo:
        total_uncompressed = functools.reduce(
            lambda x, y: x + y, [f.uncompressed for f in self.files]
        )
    ...
    def extractall(
        self, path: Optional[Any] = None, callback: Optional[ExtractCallback] = None
    ) -> None:
    ...
    def read(
        self, targets: Optional[Collection[str]] = None
    ) -> Optional[Dict[str, IO[Any]]]:
    ...
    def _writef(self, bio: IO[Any], arcname: str):
        if isinstance(bio, io.BytesIO):
            ...
            raise ValueError(
                "Unsupported file object type: please open file with Binary mode."
            )
    ...
    def reset(self) -> None:
        ...
        if self.mode == "r":
            ...
            self.worker = Worker(self.files, self.afterheader, self.header, self.mp)
            if (
                self.header.main_streams is not None
                and self.header.main_streams.unpackinfo.numfolders > 0
            ):
...
def is_7zfile(file: Union[BinaryIO, str, pathlib.Path]) -> bool:
    ...
    try:
        if (
            isinstance(file, BinaryIO)
            or isinstance(file, io.BufferedReader)
            or isinstance(file, io.IOBase)
        ) and hasattr(file, "read"):
            result = SevenZipFile._check_7zfile(file)
        elif isinstance(file, str):
            with open(file, "rb") as fp:
                result = SevenZipFile._check_7zfile(fp)
        elif (
            isinstance(file, pathlib.Path)
            or isinstance(file, pathlib.PosixPath)
            or isinstance(file, pathlib.WindowsPath)
        ):
            with file.open(mode="rb") as fp:  # noqa
                result = SevenZipFile._check_7zfile(fp)
        else:
            raise TypeError(
                "invalid type: file should be str, pathlib.Path or BinaryIO, but {}".format(
                    type(file)
                )
            )
    except OSError:
...
class Worker:
    ...
    def extract(
        ...
        if (
            ...
            src_end = (
                self.src_start + self.header.main_streams.packinfo.packpositions[-1]
            )
                ...
                if not parallel:
                    ...
                    for i in range(numfolders):
                        if skip_notarget:
                            if not any(
                                [
                                    self.target_filepath.get(f.id, None)
                    ...
                    filename: str = getattr(
                        fp, "name", ""
                    )  # do not become "" but it is for type check.
                    self.extract_single(
                        open(filename, "rb"), empty_files, path, 0, 0, q
                    )
                    concurrent_tasks = []
        ...
        for f in files:
            ...
            if fileish is None:
                ...
                if not f.emptystream:
                    if (
                        f.is_junction
                        and not isinstance(fileish, MemIO)
                        and sys.platform == "win32"
                    ):
                        with io.BytesIO() as ofp:
                            ...
                            if is_path_valid(fileish.parent.joinpath(dst), path):
                                ...
                                raise Bad7zFile(
                                    "Junction point out of target directory."
                                )
                        ...
                        with io.BytesIO() as omfp:
                            ...
                            if is_path_valid(fileish.parent.joinpath(dst), path):
                                ...
                                raise Bad7zFile(
                                    "Symlink point out of target directory."
                                )
                    else:
    ...
    def _check(self, fp, check_target, src_end):
        ...
        for f in check_target:
            with NullIO() as ofp:
                crc32 = self.decompress(
                    fp, f.folder, ofp, f.uncompressed, f.compressed, src_end
                )
            if f.crc32 is not None and crc32 != f.crc32:
                raise CrcError(crc32, f.crc32, f.filename)
    ...
    def flush_archive(self, fp, folder):
        ...
        if len(self.files) > 0:
            if "maxsize" in self.header.files_info.files[self.last_file_index]:
                self.header.files_info.files[self.last_file_index][
                    "maxsize"
                ] += foutsize
            else:
    ...
    def register_filelike(
        self, id: int, fileish: Union[MemIO, pathlib.Path, None]
    ) -> None:
