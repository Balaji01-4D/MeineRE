...
try:
    ...
    from importlib_metadata import (  # type: ignore  # noqa
        PackageNotFoundError, version)

try:
    from .c.c_ppmd import (PPMD8_RESTORE_METHOD_CUT_OFF,  # noqa
                           PPMD8_RESTORE_METHOD_RESTART, Ppmd7Decoder,
                           Ppmd7Encoder, Ppmd8Decoder, Ppmd8Encoder, PpmdError)
except ImportError:
    try:
        from .cffi.cffi_ppmd import (PPMD8_RESTORE_METHOD_CUT_OFF,  # noqa
                                     PPMD8_RESTORE_METHOD_RESTART,
                                     Ppmd7Decoder, Ppmd7Encoder, Ppmd8Decoder,
                                     Ppmd8Encoder, PpmdError)
    except ImportError:
        msg = (
            "pyppmd module: Neither C implementation nor CFFI "
...
class PpmdCompressor:
    ...
    def compress(self, data_or_str: Union[bytes, bytearray, memoryview, str]):
        if type(data_or_str) == str:
            ...
            raise ValueError(
                "Argument data_or_str is neither bytes-like object nor str."
            )
        ...
        *,
        ...
        if variant in ["I", "i"]:
            self.decoder = Ppmd8Decoder(
                max_order=max_order, mem_size=mem_size, restore_method=restore_method
            )
        else:
...
def _decompress7(
    data: Union[bytes, bytearray, memoryview], max_order: int, mem_size: int
):
