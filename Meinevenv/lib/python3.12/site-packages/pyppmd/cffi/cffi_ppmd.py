    ...
    def _setup_inBuffer(self, data):
        ...
        if self._in_begin == self._in_end:
            ...
            if avail_total < len(data):
                ...
                ffi.memmove(
                    self._input_buffer, self._input_buffer + self._in_begin, used_now
                )
            ...
            ffi.memmove(
                self._input_buffer + self._in_end, ffi.from_buffer(data), len(data)
            )
...
class Ppmd7Decoder(PpmdBaseDecoder):
    def __init__(self, max_order: int, mem_size: int):
        if mem_size > sys.maxsize:
            raise ValueError("Mem_size exceed to platform limit.")
        if (
            _PPMD7_MIN_ORDER <= max_order <= _PPMD7_MAX_ORDER
            and _PPMD7_MIN_MEM_SIZE <= mem_size <= _PPMD7_MAX_MEM_SIZE
        ):
            self.lock = Lock()
            self._init_common()
            self.ppmd = ffi.new("CPpmd7 *")
    ...
    def decode(self, data: Union[bytes, bytearray, memoryview], length: int) -> bytes:
        if not isinstance(length, int) or length < 0:
            raise PpmdError(
                "Wrong length argument is specified. It should be positive integer."
            )
        self.lock.acquire()
        in_buf, use_input_buffer = self._setup_inBuffer(data)
        if not self.inited:
            lib.ppmd7_decompress_init(
        ...
        while remaining > 0:
            ...
            self.lock.release()
            out_size = lib.ppmd7_decompress(
                self.ppmd, self.rc, out_buf, in_buf, size, self.threadInfo
            )
            self.lock.acquire()
            if out_size == -2:
                self.lock.release()
...
class Ppmd8Encoder(PpmdBaseEncoder):
    def __init__(
        self, max_order, mem_size, restore_method=PPMD8_RESTORE_METHOD_RESTART
    ):
        self.lock = Lock()
        if mem_size > sys.maxsize:
            raise ValueError("Mem_size exceed to platform limit.")
...
class Ppmd8Decoder(PpmdBaseDecoder):
    def __init__(
        self, max_order: int, mem_size: int, restore_method=PPMD8_RESTORE_METHOD_RESTART
    ):
        self._init_common()
        self.ppmd = ffi.new("CPpmd8 *")
        self.threadInfo = ffi.new("ppmd_info *")
    ...
    def _init2(self):
        lib.ppmd8_decompress_init(
            self.ppmd, self.reader, self.threadInfo, self._allocator
        )
    ...
    def decode(self, data: Union[bytes, bytearray, memoryview], length: int = -1):
        ...
        while True:
            ...
            if out_buf.pos == out_buf.size:
                out.grow(out_buf)
            self.lock.release()
            size = lib.ppmd8_decompress(
                self.ppmd, out_buf, in_buf, length, self.threadInfo
            )
            self.lock.acquire()
            if size == -1:
                self._eof = True
