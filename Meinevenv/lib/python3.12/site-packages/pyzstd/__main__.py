...
from pyzstd import PYZSTD_CONFIG, CParameter, DParameter, ZstdDict, ZstdFile
from pyzstd import __version__ as pyzstd_version
from pyzstd import (compress_stream, compressionLevel_values,
                    decompress_stream, train_dict, zstd_version)
...
def compress_option(args):
    ...
    msg = (
        ...
        " - zstd dictionary: {}\n"
        " - add checksum: {}"
    ).format(args.level, threads_msg, long_msg, args.zd, args.checksum)
    print(msg)
...
def compress(args):
    ...
    # pre-compress message
    msg = ("Compress file:\n" " - input file : {}\n" " - output file: {}").format(
        args.input.name, args.output.name
    )
    print(msg)
...
def decompress(args):
    ...
    print(
        (
            ...
            " - output file: {}\n"
            " - zstd dictionary: {}"
        ).format(args.input.name, output_name, args.zd)
    )
    ...
    t1 = time()
    ret = decompress_stream(
        args.input, args.output, zstd_dict=args.zd, option=option, callback=cb
    )
    t2 = time()
    close_files(args)
...
def tarfile_create(args):
    ...
    msg = (
        "Archive tar file:\n" " - input directory: {}\n" " - output file: {}"
    ).format(args.tar_input_dir, args.output.name)
    print(msg)
    ...
    t1 = time()
    with ZstdTarFile(
        args.output, mode="w", level_or_option=option, zstd_dict=args.zd
    ) as f:
    ...
    msg = (
        "Archiving succeeded, {:.2f} seconds.\n"
        "Input ~{:,} bytes, output {:,} bytes, ratio {:.2f}%.\n"
...
def tarfile_extract(args):
    ...
    ).format(args.input.name, args.tar_output_dir, args.zd)
    print(msg, flush=True)

    # extract
    t1 = time()
    with ZstdTarFile(
        args.input, mode="r", zstd_dict=args.zd, level_or_option=option
    ) as f:
    ...
    if uncompressed_size != 0:
        ...
        ratio = 100.0
    msg = (
        "Extraction succeeded, {:.2f} seconds.\n"
        "Input {:,} bytes, output ~{:,} bytes, ratio {:.2f}%.\n"
    ...
    class RangeAction(argparse.Action):
        def __call__(self, parser, args, values, option_string=None):
            ...
            if not (min <= v <= max):
                ...
                # message
                msg = (
                    "{}{} value should: {} <= v <= {}. " "provided value is {}."
                ).format(bits, option_string, min, max, v)
...
def parse_arg():
    ...
    g.add_argument(
        ...
        help="compression level, range: [{},{}], default: {}.".format(
            compressionLevel_values.min,
...
def main():
    print(
        "*** pyzstd module v{}, zstd library v{}. ***\n".format(
            pyzstd_version, zstd_version
        )
    )
