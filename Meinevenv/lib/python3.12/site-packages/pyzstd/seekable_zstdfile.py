...
from pyzstd.zstdfile import (_MODE_CLOSED, _MODE_READ, _MODE_WRITE, PathLike,
                             ZstdDecompressReader, ZstdFile, io)
...
class SeekTable:
    ...
    def load_seek_table(self, fp, seek_to_0):
        ...
        if descriptor & 0b01111100:
            msg = (
                "In Zstandard Seekable Format version %s, the "
        ...
        for idx in range(frames_number):
            ...
            if compressed_size == 0 and decompressed_size != 0:
                msg = (
                    "Wrong seek table. The index %d frame (0-based) "
            ...
            if self._full_c_size > fsize - skippable_frame_size:
                msg = (
                    "Wrong seek table. Since index %d frame (0-based), "
    ...
    def write_seek_table(self, fp):
        ...
        if self._frames_count > 0xFFFFFFFF:
            ...
            warn(
                (
                    "SeekableZstdFile's seek table has %d entries, "
...
class SeekableDecompressReader(ZstdDecompressReader):
    ...
    def seek(self, offset, whence=0):
        ...
        if (
            new_frame == old_frame
            and offset >= self._decomp.pos
            and self._fp.tell() >= c_pos
        ):
            pass
        else:
            # Jump
