...
from psutil.tests import (APPVEYOR, GITHUB_ACTIONS, HAS_BATTERY, IS_64BIT, PY3,
                          PYPY, TOLERANCE_DISK_USAGE, TOLERANCE_SYS_MEM,
                          PsutilTestCase, mock, pytest, retry_on_failure, sh,
...
@pytest.mark.skipif(GITHUB_ACTIONS and not PY3, reason="pywin32 broken on GITHUB + PY2")
    ...
    def test_nic_names(self):
        ...
        for nic in nics:
            if "pseudo-interface" in nic.replace(" ", "-").lower():
                continue
            if nic not in out:
                raise self.fail("%r nic wasn't found in 'ipconfig /all' output" % nic)
            ...
            for wmi_part in wmi_parts:
                if ps_part.device.replace("\\", "") == wmi_part.DeviceID:
                    ...
                    if abs(usage.free - wmi_free) > 10 * 1024 * 1024:
                        raise self.fail("psutil=%s, wmi=%s" % (usage.free, wmi_free))
                    break
            else:
