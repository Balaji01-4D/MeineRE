...
def dialog(title, message, icon, buttons=["OK"]):
    script = [
        ...
        icon,
        "buttons",
        "{" + ",".join(quote(btn) for btn in buttons) + "}",
    ]
    if title:
        script.append("with title " + quote(title))
...
def open_file(title, filetypes, multiple=False):
    script = ["choose file"]
    if title:
        script.append("with prompt " + quote(title))
    if filetypes:
        oftype = []
    ...
    if multiple:
        ...
        code, out, err = osascript(
            f'set ps to ({" ".join(script)})\rrepeat with p in ps\r log (POSIX path of p)\rend repeat'
        )
        if code:
            return []
