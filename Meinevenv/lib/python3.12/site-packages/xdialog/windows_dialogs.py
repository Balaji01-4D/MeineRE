...
def directory(title):
    ...
    try:
        ...
        if hr < 0:
            raise OSError("Failed to create dialog")

        # Set options
        flags = UINT(0)
        hr = ifd.contents.lpVtbl.contents.GetOptions(ifd, ctypes.byref(flags))
        if hr < 0:
            raise OSError("Failed to get options")
        ...
        if title is not None:
            ifd.contents.lpVtbl.contents.SetTitle(ifd, title)
        ...
        try:
            ...
            if hr < 0:
                raise OSError("Failed to get result of directory selection")
            wstr = LPWSTR()
            hr = resultIf.contents.lpVtbl.contents.GetDisplayName(
                resultIf, 0x80058000, ctypes.byref(wstr)
            )
            if hr < 0:
                raise OSError("Failed to get display name from shell item")
            val = wstr.value
