...
def zenity(typ, filetypes=None, **kwargs) -> Tuple[int, str]:
    ...
    proc = subprocess.Popen(
        args, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, shell=False
    )
...
def yesnocancel(title, message):
    r = zenity(
        "question", title=title, text=message, extra_button="No", cancel_label="Cancel"
    )
...
def okcancel(title, message):
    r = zenity(
        "question", title=title, text=message, ok_label="Ok", cancel_label="Cancel"
    )[0]
